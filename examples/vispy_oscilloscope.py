...
class MicrophoneInputLayer(prt.ProducerMixin, prt.ThreadLayer):

    def __init__(self, rate=44100, chunk_size=1024, *args, **kwargs):
    ...
    def initialize(self):
        ...
        self.stream = self.pyaudio.open(format=pyaudio.paInt16,
               ...
               stream_callback=self.on_new_audio_frame)
        self.stream.start_stream()

    def on_new_audio_frame(self, data, frame_count, time_info, status):
        data = np.fromstring(data, 'int16')
        self.supply_input(data)
        if self.stop:
...
class OscilloscopeWrapper(prt.TransformMixin, prt.ThreadLayer):
    def __init__(self, port_in, n_lines=100, line_size=1024, dx=1e-4,
        ...
        self.scope = scene.ScrollingLines(n_lines=1, line_size=line_size, dx=0.8/line_size/1000,
                             cell_size=(1, 8), columns=1, parent=parent)


    def transform(self, data):
        # self.scope.update_scope(data)
        self.scope.roll_data(np.atleast_2d(data)/1000)
...
mic = MicrophoneInputLayer()
n_fft_frames = 8
fft_samples = mic.chunk_size * n_fft_frames
...
scope = OscilloscopeWrapper(mic, line_size=mic.sample_rate//2, dx=1.0/mic.sample_rate/100,
                     parent=view1.scene)

# view2 = grid.add_view(row=1, col=1, camera='panzoom', border_color='grey')
# view2.camera.rect = (0.5, -0.5e6, np.log10(mic.sample_rate/2), 5e6)
...
#                         dx=mic.sample_rate/fft_samples,
...
prt.LayerManager.session().start()#main_thread=scope)
app.run()
